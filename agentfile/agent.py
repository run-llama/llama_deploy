...
class AgentServer:
    def __init__(self, agent: AgentRunner, running: bool = True, description: str = "Agent Server", step_interval: int = 0.1):
        self.agent = agent
        ...
        self.app.add_api_route("/task", self.create_task, methods=["POST"], tags=["Tasks"])
        self.app.add_api_route("/tasks", self.get_tasks, methods=["GET"], tags=["Tasks"])
        self.app.add_api_route("/tasks/{task_id}", self.get_task, methods=["GET"], tags=["Tasks"])
        self.app.add_api_route("/completed_tasks", self.get_completed_tasks, methods=["GET"], tags=["Tasks"])
        self.app.add_api_route("/tasks/{task_id}/output", self.get_task_output, methods=["GET"], tags=["Tasks"])
        self.app.add_api_route("/tasks/{task_id}/steps", self.get_task_steps, methods=["GET"], tags=["Tasks"])
        self.app.add_api_route("/tasks/{task_id}/completed_steps", self.get_completed_steps, methods=["GET"], tags=["Tasks"])

        self.app.add_api_route("/description", self.get_description, methods=["GET"], tags=["Agent State"])
        self.app.add_api_route("/messages", self.get_messages, methods=["GET"], tags=["Agent State"])
        self.app.add_api_route("/toggle_agent_running", self.toggle_agent_running, methods=["POST"], tags=["Agent State"])
        self.app.add_api_route("/is_worker_running", self.is_worker_running, methods=["GET"], tags=["Agent State"])
        self.app.add_api_route("/reset_agent", self.reset_agent, methods=["POST"], tags=["Agent State"])
    ...
    async def toggle_agent_running(self, state: Literal["running", "stopped"]) -> Dict[str, bool]:
